...
if (Test-Port 8000) {
    ...
    Start-Process -FilePath $venvPython -ArgumentList "-m uvicorn app.main:app --host 0.0.0.0 --port 8000" -WindowStyle Hidden -WorkingDirectory $backendDir
    if (-not (Wait-ForPort 8000 "Backend" 30)) { exit 1 }
    # Seed data if graph is empty
    try {
        $resp = Invoke-RestMethod -Uri "http://localhost:8000/api/v1/query/stats" -TimeoutSec 5
        if ($resp.total_nodes -eq 0) {
            ...
            Invoke-RestMethod -Uri "http://localhost:8000/api/v1/batches" -Method POST -ContentType "application/json" -Body $body -TimeoutSec 30 | Out-Null
