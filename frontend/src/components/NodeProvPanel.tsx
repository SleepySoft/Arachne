...
import { ArrowRight, FileText, History, X, Code } from "lucide-react";
import { listProvStatementsByNode, getProvNByNode } from "@/services/api";
...
export function NodeProvPanel({ nodeId, nodeName, onClose }: NodeProvPanelProps) {
  ...
  const { data: provnText, isLoading: provnLoading } = useQuery({
    queryKey: ["provn", nodeId],
    queryFn: () => getProvNByNode(nodeId),
    staleTime: 60_000,
    enabled: showSource,
  });
  ...
  return (
    <div className="flex h-full flex-col bg-slate-900 p-4">
      ...
      {!showSource && !isLoading && items.length === 0 && (
      ...
      {showSource && (
        <div className="mt-4 flex-1 overflow-auto rounded border border-slate-800 bg-slate-950 p-3">
          {provnLoading && (
            <div className="text-center text-xs text-slate-500">加载 PROV-N …</div>
          )}
          {!provnLoading && (
      ...
      <div className="mt-3 border-t border-slate-800 pt-2 text-[10px] text-slate-600">
        ...
        PROV 声明为类型级溯源断言，以 PROV-N 格式存储于文件，与产业图边独立。
      </div>
    </div>
  );
}
