...
根据公司所依赖和产出的实体和服务，通过产业视图，计算出公司在供应链上的上下流关系。
...
class CompanyRelationSubtype(str, Enum):
...
# ---------------------------------------------------------------------------
# Company view Node (投影视图)
# ---------------------------------------------------------------------------

class CompanyViewNode(BaseModel):
    """
    公司视图中的节点。
    这不是 IndustrialNode，而是"某公司在视图上下文中暴露的产业节点"的视图。
    """
...
class CompanyViewEdge(BaseModel):
    """
    公司视图中的边。
...
class CompanyViewRelation(BaseModel):
    """
    公司视图域内的推导关系。
    只存在于视图快照中，不属于核心产业图。
    """
    ...
    relation_type: CompanyRelationType = Field(default=CompanyRelationType.INFERRED_INDUSTRIAL)
    relation_subtype: Optional[CompanyRelationSubtype] = None
...
class CompanyView(BaseModel):
    ...
    每次计算生成一个独立版本，旧版本保留。
    """
    company_view_uuid: UUID = Field(default_factory=uuid4)
    company_view_id: str = Field(..., pattern=r"^[a-z][a-z0-9_]*$")
    ...
    # 展开时填充（不参与数据库存储）
    nodes: List[CompanyViewNode] = Field(default_factory=list)
    edges: List[CompanyViewEdge] = Field(default_factory=list)
    relations: List[CompanyViewRelation] = Field(default_factory=list)


class PaginatedCompanyViews(BaseModel):
    total: int
    page: int
    page_size: int
