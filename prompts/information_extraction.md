...
只提取以下类型的**稳定产业实体**：
- **材料（material）**：硅料、锂矿石、钢铁、塑料、光刻胶、电解液
- **零件/部件（part）**：轴承、齿轮、外壳、连接器、电极片、隔膜
- **器件/装置（device）**：传感器、激光器、芯片、电池电芯、摄像头、电机
- **装备/设备（equipment）**：光刻机、数控机床、焊接机器人、三坐标测量机
- **系统（system）**：智能手机、服务器、电动汽车、风力发电机、电池管理系统
- **软件（software）**：操作系统、数据库、仿真软件、感知算法、SDK
- **平台（platform）**：云计算平台、工业互联网平台、AI 训练平台
- **基础设施（infrastructure）**：5G 网络、数据中心、电网、高速公路
- **工艺/流程（process）**：光刻、刻蚀、涂布、注塑、模型训练
- **服务（service）**：云计算服务、运维服务、检测认证服务
- **技术能力（technology_capability）**：多传感器融合、机器视觉、高压快充
- **标准/协议（standard）**：USB、CAN、MQTT、ISO 26262
- **数据资产（data_asset）**：高精地图、训练数据集、语料库、缺陷样本库
...
| 关系类型 | 判断标准 | 示例 |
|---------|---------|------|
| material_input | A 是 B 的物理原料，经加工后成为 B 的一部分 | 硅料 → 芯片，锂矿石 → 锂电池 |
| structural_composition | A 是 B 的结构组成部分 | 轴承 → 电机，PCB → 智能手机 |
| energy_input | A 为 B 提供运行所需能量 | 电力 → 电动汽车，燃料 → 数据中心 |
| information_input | A 向 B 提供数据、信号、控制信息 | 传感器 → 控制系统，高精地图 → 自动驾驶系统 |
| equipment_enablement | A 是用于生产/检测 B 的装备 | 光刻机 → 晶圆制造，注塑机 → 塑料制品 |
| capability_enablement | A 为 B 提供基础能力，使 B 能运行 | 操作系统 → 应用软件，EDA软件 → 芯片设计 |
| service_provision | A 以服务形式持续支持 B | 云计算服务 → SaaS应用，运维服务 → 数据中心 |
| process_output | A 过程产出的结果 | 晶圆制造 → 晶圆，电池化成 → 电芯 |
