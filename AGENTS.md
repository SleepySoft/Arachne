...
- **Embedded arachne-flow editor panel in main workspace**: Added `FlowEditorPanel`, an inline YAML editor shown as a right-side panel inside the industrial graph workspace when the arachne_flow engine is active. The FlowSidebarPanel now has an "编辑流程" button that opens the panel without navigating away, so the total graph remains visible. The panel includes the editor, save/format/collapse controls, a small live preview canvas, and a disabled "从图添加（预留）" placeholder for future graph-level editing (which will ultimately also land in a single file). After saving, the main graph recompiles the selected flows. All 72 backend tests pass; frontend `npm run build` passes.
