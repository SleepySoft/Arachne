...
- **Reasoning query scope cleanup**: The query scope dropdown in `ReasoningPage` exposed edge and claim scopes, but reasoning tasks only accept node-like sources and the backend does not implement the `claim` scope. Limited the dropdown to `industrial_node`, `factual_node`, `company`, and `industry`. The query input label/placeholder and the example-fill button now adapt to the selected scope (e.g. “公司名称 / 输入公司名/别名/股票代码” for company scope).
