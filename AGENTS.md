...
### Recent Changes
- **Topology-aware reasoning**: Ontology edges (`is_a`, `part_of`, `related_term`, `variant_of`, `alias_of`) are no longer mixed into supply-chain traversal by default. `association` and `bottleneck_detection` now traverse `INDUSTRIAL_FLOW` edges only unless the caller explicitly requests ontology namespaces. Added `expand_ontology` task parameter (CLI `--expand-ontology`, frontend toggle) that expands source nodes via ontology edges before flow traversal, without returning ontology edges as flow paths. Also fixed a missing `is_test` parameter in `neo4j_storage.create_ontology_edge`.
