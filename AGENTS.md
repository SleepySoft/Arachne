...
- **Dual Neo4j instances**: Community Edition does not support multiple databases, so run two Neo4j processes on different ports (main: `7687`, flow: `7688`).
- **arachne-flow engine**: implement `app/engines/arachne_flow/` as a read-only engine that compiles YAML triples into the second Neo4j instance via `:ARACHNE_FLOW` relationships.
- **Reasoning is core-led, engine-enabled**: finalize core task contracts and generic graph algorithms; add a `ReasoningAdapter` per engine for edge-type semantics and engine-specific task handlers.
- **Ontology/topology service**: promote `alias_of` / `is_a` / `part_of` / `variant_of` expansion into a shared `TopologyService` in core; ontology data stays in the main Neo4j instance and is shared across engines.
- **Pass-through subsystem APIs**: expose per-engine routers under `/api/v1/engines/{engine_name}/*` for engine-specific operations (e.g., arachne-flow compile/validate/status).
...
*Last updated: 2026-07-16 14:35 CST*
