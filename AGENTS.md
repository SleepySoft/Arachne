...
- **PROV statement storage backend**: Implemented type-level PROV assertion storage as JSON files under `data/prov_statements/{node_id}.prov.json`. Added `backend/app/models/prov_schema.py`, `backend/app/services/prov_storage.py`, `backend/app/routers/prov.py` mounted at `/api/v1/prov`, and `backend/tests/test_prov_storage.py`. Supports `used`, `wasGeneratedBy`, `wasDerivedFrom`, and other PROV relations attached to industrial nodes, independent of the Neo4j graph. The legacy `prov_statements` PostgreSQL table was removed after migrating its 30 rows into files. Fixed `statement_id` round-trip so GET/PUT/DELETE by composite ID works. The earlier W3C PROV-N experiment is deprecated: `backend/app/services/prov_n.py` and the `/prov/nodes/{id}/provn` endpoints are kept in the repository but commented out of the main code path; existing `.provn` files were converted back to `.prov.json`. Design recorded in `docs/prov_overlay_design.md`.
