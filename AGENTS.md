...
- **PROV statement storage backend**: Implemented type-level PROV assertion storage as plain JSON files under `data/prov_statements/{node_id}.prov.json`. Added `backend/app/models/prov_schema.py`, `backend/app/services/prov_storage.py`, `backend/app/routers/prov.py` mounted at `/api/v1/prov`, and `backend/tests/test_prov_storage.py`. Supports `used`, `wasGeneratedBy`, `wasDerivedFrom`, and other PROV relations attached to industrial nodes, independent of the Neo4j graph. The legacy `prov_statements` PostgreSQL table was removed after migrating its 30 rows into files. Design recorded in `docs/prov_overlay_design.md`.
...
- **PROV-N storage format**: Replaced the JSON file format for PROV statements with W3C PROV-N text documents stored as `data/prov_statements/{node_id}.provn`. Added `backend/app/services/prov_n.py` with a parser and serializer for the supported subset (`entity`/`activity`/`agent` declarations and `used`/`wasGeneratedBy`/`wasDerivedFrom`/`wasAttributedTo`/`wasAssociatedWith`/`actedOnBehalfOf` relations). Updated `backend/app/services/prov_storage.py` to read/write PROV-N files while keeping the legacy JSON loader as a fallback, and added `GET /api/v1/prov/nodes/{node_id}/provn` to expose raw PROV-N text. Updated `frontend/src/components/NodeProvPanel.tsx` with a "关系列表 / PROV-N" tab switch and `frontend/src/services/api.ts` with `getProvNByNode`. Created and ran `scripts/migrate_prov_to_provn.py` to convert all 22 existing `.prov.json` files to `.provn`. Updated `docs/prov_overlay_design.md`. Frontend `npm run build` passes.
