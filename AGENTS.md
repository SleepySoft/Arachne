...
- **Recompile workflow UI**: flows are read-only in the UI; add a "重新编译" action in the flow sidebar that calls `POST /api/v1/flows/{flow_id}/compile` after YAML edits.
